...
import { deriveMasterKeyHexWithWorkerFallback } from '../services/pbkdf2DeriveService';
...
const secureLocalStorage = {
  ...
  getItem: (key: string): null | string => {
    try {
      ...
      return globalThis.atob(storedValue); // base64 decoding
    } catch (e) {
      return null;
    }
  },
  ...
  setItem: (key: string, value: string): void => {
    try {
      ...
      const obfuscated = globalThis.btoa(value); // base64 encoding
...
const useAuthStore = create<AuthState>((set, get) => ({
  checkPersistedEncryptionKey: () => {
    ...
    if (storedPasswordHash && storedKeyHash) {
      // Hashes exist from a prior session; user must unlock again — no auto-fill of the master password.
    }
  },
  clearEncryptionKey: () => {
    const user = get().user;
  ...
  setMasterPassword: async (password: string): Promise<void> => {
    ...
    try {
      set({ isLoading: true });
      const keyString = await deriveMasterKeyHexWithWorkerFallback(
        password,
        user.uid
      );
