...
interface CredentialState {
  ...
  globalSearchCredentials: DecryptedCredential[];
  globalSearchIndexLoading: boolean;
  isLoading: boolean;
  refreshGlobalSearchIndex: () => Promise<void>;
...
/** Loads and decrypts every credential across all projects (used by All Credentials + global search). */
const loadAllDecryptedCredentialsForUser = async (
  userId: string,
  encryptionKey: string,
): Promise<{ decryptionErrors: number; items: DecryptedCredential[] }> => {
  ...
  const credentialSnapshots: { snap: typeof projectsSnapshot }[] = [];

  for (let i = 0; i < projectDocs.length; i += listConcurrency) {
    const chunk = projectDocs.slice(i, i + listConcurrency);
    const chunkSnaps = await Promise.all(
      chunk.map((projectDoc) =>
        getDocs(
          collection(db, 'users', userId, 'projects', projectDoc.id, 'credentials')
        )
      )
    );
    for (const snap of chunkSnaps) {
      credentialSnapshots.push({ snap });
    }
  }

  for (const { snap: credentialsSnapshot } of credentialSnapshots) {
  ...
  return { decryptionErrors, items: fetchedCredentials };
};
...
const useCredentialStore = create<CredentialState>((set, get) => ({
  ...
  clearCredentials: () => {
    set({ credentials: [], error: null, globalSearchCredentials: [], isLoading: false });
  },
  credentials: [],
  globalSearchCredentials: [],
  globalSearchIndexLoading: false,
  isLoading: false,
  ...
  deleteCredentialsByProject: async (projectId: string) => {
    ...
    try {
      ...
      set(state => ({
        ...
        globalSearchCredentials: state.globalSearchCredentials.filter(c => c.projectId !== projectId),
        isLoading: false
      }));
    } catch (e: unknown) {
      set({ error: e instanceof Error ? e : new Error(String(e)), isLoading: false });
  ...
  fetchAllCredentials: async () => {
    ...
    const encryptionKey = useAuthStore.getState().encryptionKey;
    if (!currentUser || !encryptionKey) {
      set({ credentials: [], error: new Error('User not authenticated or master password not set.'), globalSearchCredentials: [], isLoading: false });
      return;
    }
    set({ error: null, isLoading: true });
    try {
      const { decryptionErrors, items } = await loadAllDecryptedCredentialsForUser(currentUser.uid, encryptionKey);
      set({ credentials: items, globalSearchCredentials: items, isLoading: false });
      if (decryptionErrors > 0 && items.length === 0) {
        set({
  ...
  refreshGlobalSearchIndex: async () => {
    const currentUser = auth.currentUser;
    const encryptionKey = useAuthStore.getState().encryptionKey;
    if (!currentUser || !encryptionKey) {
      set({ globalSearchCredentials: [], globalSearchIndexLoading: false });
      return;
    }
    set({ globalSearchIndexLoading: true });
    try {
      const { items } = await loadAllDecryptedCredentialsForUser(currentUser.uid, encryptionKey);
      set({ globalSearchCredentials: items, globalSearchIndexLoading: false });
    } catch {
      set({ globalSearchCredentials: [], globalSearchIndexLoading: false });
    }
  },
