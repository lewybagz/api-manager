...
const fieldBase =
  "block w-full rounded-xl border border-zk-border bg-zk-base/80 py-3 pl-12 pr-4 font-zk-sans text-sm text-zk-text placeholder:text-zk-muted/50 transition-colors focus:border-rose-500/40 focus:outline-none focus:ring-2 focus:ring-rose-500/25 sm:text-base";

const fieldPassword =
  "block w-full rounded-xl border border-zk-border bg-zk-base/80 py-3 pl-12 pr-12 font-zk-sans text-sm text-zk-text placeholder:text-zk-muted/50 transition-colors focus:border-rose-500/40 focus:outline-none focus:ring-2 focus:ring-rose-500/25 sm:text-base";

const PwLoginPage: React.FC = () => {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("appType", "pw");
    } catch {
      void 0;
    }
    applyBranding();
  ...
  useEffect(() => {
    ...
    if (userDoc.appType === "api") {
      void signOut(auth);
      authSetError(
        new Error(
          "This account uses the credential manager. Sign in there instead."
        )
      );
      return;
    }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-transparent font-zk-sans text-zk-text">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-zk-muted">One moment…</p>
        </div>
      </div>
    );
  }
  ...
  const tabActive =
    "rounded-lg bg-rose-500/90 py-2 text-sm font-semibold text-white shadow-sm";
  const tabIdle =
    "rounded-lg py-2 text-sm font-semibold text-zk-muted transition-colors hover:bg-zk-elevated/50 hover:text-zk-text";

  const primaryBtn =
    "flex w-full items-center justify-center gap-2 rounded-xl bg-rose-500 py-3 text-sm font-semibold text-white transition-colors hover:bg-rose-600 disabled:cursor-not-allowed disabled:opacity-50";
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-transparent px-4 py-10 font-zk-sans text-zk-text sm:px-6">
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-gradient-to-br from-rose-950/30 via-zk-base to-zk-surface"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -left-24 top-1/3 h-64 w-64 rounded-full bg-rose-500/10 blur-3xl"
      />
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl border border-zk-border bg-zk-elevated/40 p-6 shadow-[0_24px_64px_-24px_rgba(0,0,0,0.55)] backdrop-blur-xl sm:p-8">
        <div className="flex flex-col items-center">
          ...
          <p className="mb-6 text-center text-sm leading-relaxed text-zk-muted">
            Save and find your passwords in one calm place.
          </p>
          ...
          <div className="mb-8 flex w-full gap-1 rounded-xl border border-zk-border bg-zk-base/50 p-1">
            <button
          ...
          {!isRegister && (
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 rounded-md p-1 text-zk-muted transition-colors hover:bg-zk-elevated/80 hover:text-rose-200"
                      disabled={isSubmitting}
                      onClick={() => {
                        setShowPassword((v) => !v);
                      }}
                ...
                <div className="flex items-center justify-between gap-3">
                  <label className="flex cursor-pointer items-center gap-2 text-sm text-zk-muted">
                    <input
                      checked={rememberMe}
                      className="rounded border-zk-border bg-zk-base text-rose-500 focus:ring-rose-500/30"
                      onChange={(e) => {
              ...
              <button className={primaryBtn} disabled={isSubmitting} type="submit">
                {isSubmitting ? (
                  <>
